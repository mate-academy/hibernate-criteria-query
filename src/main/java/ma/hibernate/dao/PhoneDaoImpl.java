...
public class PhoneDaoImpl extends AbstractDao implements PhoneDao {
    ...
    public List<Phone> findAll(Map<String, String[]> params) {
        try (Session session = factory.openSession()) {
            ...
            Predicate finalPredicate = cb.isTrue(cb.literal(true));//the hardest line
            for (Map.Entry<String, String[]> pairs : params.entrySet()) {
                CriteriaBuilder.In<String> predicate = cb.in(phoneRoot.get(pairs.getKey()));
                for (String attribute : pairs.getValue()) {
