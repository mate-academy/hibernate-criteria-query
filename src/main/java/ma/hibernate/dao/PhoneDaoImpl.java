...
public class PhoneDaoImpl extends AbstractDao implements PhoneDao {
    ...
    public List<Phone> findAll(Map<String, String[]> params) {
        try (Session session = factory.openSession()) {
            ...
            throw new RuntimeException("Can't get all phones with parameters: "
                    + params.entrySet().stream().map(p -> p.getKey() + ":"
                    + Arrays.toString(p.getValue())).collect(Collectors.joining(", ")), e);
        }
    }
}
