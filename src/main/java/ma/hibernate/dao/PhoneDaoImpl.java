    ...
    public List<Phone> findAll(Map<String, String[]> params) {
        try (Session session = factory.openSession()) {
            ...
            for (Map.Entry<String, String[]> param: params.entrySet()) {
                CriteriaBuilder.In<Object> criteria = criteriaBuilder.in(root.get(param.getKey()));
                for (String param2: param.getValue()) {
                    criteria.value(param2);
                }
                criteriaBuilders.add(criteria);
            }
            Predicate finalPredicate = criteriaBuilder
                    .and(criteriaBuilders.toArray(new Predicate[0]));
