...
public class PhoneDaoImpl extends AbstractDao implements PhoneDao {
    ...
    public Phone create(Phone phone) {
        ...
        try {
            ...
            throw new RuntimeException("Couldn't create country " + phone, e);
        } finally {
            if (session != null) {
                session.close();
            }
        ...
        try (Session session = factory.openSession()) {
            ...
            if (params.size() > 0) {
                ...
                for (Map.Entry<String, String[]> currentEntry : params.entrySet()) {
                    String criteria = currentEntry.getKey();
                    CriteriaBuilder.In<String> currentPredicate
                            = criteriaBuilder.in(phoneRoot.get(criteria));
                    for (String currentValue : currentEntry.getValue()) {
                        currentPredicate.value(currentValue);
                    }
