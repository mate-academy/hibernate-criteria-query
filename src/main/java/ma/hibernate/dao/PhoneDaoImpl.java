        ...
        try (Session session = factory.openSession()) {
            ...
            for (Map.Entry<String, String[]> entry : params.entrySet()) {
                ...
                for (String val : entry.getValue()) {
                    paramsPredicate.value(val);
                }
                andPredicate = criteriaBuilder.and(andPredicate, paramsPredicate);
            }
            return session.createQuery(phoneCriteriaQuery.where(andPredicate)).getResultList();
        } catch (Exception e) {
            throw new RuntimeException("Can't find all phones by params.", e);
        }
    }
}
