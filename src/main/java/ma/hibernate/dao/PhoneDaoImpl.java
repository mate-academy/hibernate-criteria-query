    ...
    public List<Phone> findAll(Map<String, String[]> params) {
        try (Session session = factory.openSession()) {
            ...
            for (Map.Entry<String, String[]> entry : params.entrySet()) {
                CriteriaBuilder.In<Object> fieldPredicate
                        = criteriaBuilder.in(root.get(entry.getKey()));
                Arrays.stream(entry.getValue()).forEach(fieldPredicate::value);
                finalPredicate = criteriaBuilder.and(finalPredicate, fieldPredicate);
            }
            query.where(criteriaBuilder.and(finalPredicate));
            return session.createQuery(query).getResultList();
        } catch (Exception e) {
            throw new DataProcessingException("Can't find phones by our params " + params, e);
        }
    }
}
