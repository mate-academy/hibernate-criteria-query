    ...
    public List<Phone> findAll(Map<String, String[]> params) {
        try (Session session = factory.openSession()) {
            ...
            for (Map.Entry<String, String[]> entry : params.entrySet()) {
                CriteriaBuilder.In<String> stringCriteria =
                        criteria.in(phoneRoot.get(entry.getKey()));
                for (String paramValue : entry.getValue()) {
                    listOfPredicates.add(stringCriteria.value(paramValue));
                }
            }
            Predicate[] predicates = new Predicate[listOfPredicates.size()];
            listOfPredicates.toArray(predicates);
