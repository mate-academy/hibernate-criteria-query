    ...
    public List<Phone> findAll(Map<String, String[]> params) {
        try (Session session = factory.openSession()) {
            ...
            for (Map.Entry<String, String[]> pairs : params.entrySet()) {
                CriteriaBuilder.In<Object> categoryPredicate = cb.in(phoneRoot.get(pairs.getKey()));
                for (String param: pairs.getValue()) {
                    categoryPredicate.value(param);
                }
                result = cb.and(result, categoryPredicate);
            }
            query.where(result);
